...
class InvestorVC: UIViewController , UITextFieldDelegate {
    ...
    let StackVertical = UIStackView(arrangedSubviews: [ProjectBrief,Partnership,InvestmentBudget,StartUpView,InvestmentInterestedIn,LengthOfBusiness])
    ...
    StackVertical.frame = CGRect(x: ControlX(20), y: ControlY(180), width: view.frame.width - ControlX(40), height: ControlHeight(360))

    ViewScroll.addSubview(UploadFiles)
    ViewScroll.addSubview(CommentsTF)

    ViewScroll.addSubview(SubmitButton)
    UploadFiles.frame = CGRect(x: ControlX(20), y: ControlY(590), width: ControlWidth(190), height: ControlHeight(36))

    CommentsTF.frame = CGRect(x: ControlX(20), y: ControlY(660), width: view.frame.width - ControlX(40), height: ControlHeight(30))

    SubmitButton.frame = CGRect(x: view.center.x - ControlWidth(60), y: ControlY(760), width: ControlWidth(120), height: ControlHeight(40))
    SubmitButton.layer.cornerRadius = SubmitButton.frame.height / 2

    InvestmentBudget.delegate = self
    ...
    lazy var Partnership : UITextField = {
        let tf = FloatingTF()
        tf.attributedPlaceholder = NSAttributedString(string: "Partnership".localizable, attributes:[.foregroundColor: LabelForeground])
        return tf
    }()

    lazy var InvestmentBudget : UITextField = {
        let tf = FloatingTF()
        tf.attributedPlaceholder = NSAttributedString(string: "InvestmentBudget".localizable, attributes:[.foregroundColor: LabelForeground])
        tf.keyboardType = .phonePad
        return tf
    }()

    lazy var InvestmentInterestedIn : UITextField = {
        let tf = FloatingTF()
        tf.attributedPlaceholder = NSAttributedString(string: "Investment interested in".localizable, attributes:[.foregroundColor: LabelForeground])
        return tf
    }()
...
extension InvestorVC : UIDocumentPickerDelegate {
...
///----------------------------------------------
extension InvestorVC {
    
    func AddRequest(FileUid:String) {
     guard let url = defaults.string(forKey: "API") else{return}
    let api = "\(url + PhoneInvestorsRequest)"
    ...
    guard let Partnership = Partnership.text else {return}
    guard let Investment = InvestmentBudget.text else {return}
    guard let InvestmentInterestedIn = InvestmentInterestedIn.text else {return}

    let StartUp = StartUpView.YesButton.isOn
    let lengthOfBusiness = LengthOfBusiness.text ?? ""
    let Comments = CommentsTF.text ?? ""

    if ProjectBrief.TextNull() == false || Partnership.TextNull() == false || Investment.TextNull() == false || InvestmentInterestedIn.TextNull() == false {
    ...
    let parameters: [String: Any] = [
        ...
        "sysToken": sysToken,
        "proId": IdAdd,
        "projectSectorTypId": SectorTypId,
        "projectBrief": ProjectBrief,
        "partnership": Partnership,
        "investmentBudget": Investment,
        "startup": StartUp,
        "investmentInterestedIn": InvestmentInterestedIn,
