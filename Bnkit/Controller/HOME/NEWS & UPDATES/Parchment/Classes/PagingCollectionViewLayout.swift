  ...
  private func createLayoutAttributes() {
    ...
    for index in 0..<self.view.numberOfItems(inSection: 0) {
      ...
      if sizeCache.implementsWidthDelegate {
        ...
        attributes.frame = CGRect(x: x, y: y, width: width, height: options.menuItemSize.height - options.menuInsets.top)
      } else {
        switch (options.menuItemSize) {
        case let .fixed(width, height):
            attributes.frame = CGRect(x: x, y: y, width: width, height: height - options.menuInsets.top)
        case let .sizeToFit(minWidth, height):
          attributes.frame = CGRect(x: x, y: y, width: minWidth, height: height - options.menuInsets.top)
        case let .selfSizing(estimatedWidth, height):
          if let actualWidth = widthCache[pagingItem.identifier] {
            attributes.frame = CGRect(x: x, y: y, width: actualWidth, height: height - options.menuInsets.top)
          } else {
            attributes.frame = CGRect(x: x, y: y, width: estimatedWidth, height: height - options.menuInsets.top)
          }
        }
      }
