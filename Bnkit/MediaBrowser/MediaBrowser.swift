...
@objcMembers public class MediaBrowser: UIViewController, AVPlayerViewControllerDelegate {
    ...
    /// Hide time inerval
    public var delayToHideElements = TimeInterval(2.0)
    ...
    public var loadingIndicatorFont = UIFont.systemFont(ofSize: ControlWidth(10))
...
//                    action: #selector(doneButtonPressed))
                ...
                doneButton = UIBarButtonItem(image: UIImage(named: "close")?.imageWithImage(scaledToSize: CGSize(width: ControlWidth(20), height: ControlWidth(20))) ?? UIImage(), style: .plain, target: self, action:  #selector(doneButtonPressed))
        ...
        if previousButton != nil && nextButton != nil && photos > 1 {
            ...
            items.append("lang".localizable == "ar" ? nextButton! : previousButton!)
            items.append(flexSpace)
            items.append("lang".localizable != "ar" ? nextButton! : previousButton!)
            items.append(flexSpace)
        } else {
    ...
    open func updateNavigation() {
        ...
        if let gc = gridController {
            ...
            if nil == title {
                ...
                title = "lang".localizable == "ar" ?  "\(currentPageIndex + 1) \(str) \(numberOfMedias)".NumAR() : "\(currentPageIndex + 1) \(str) \(numberOfMedias)"
            }
        } 
    ...
    func playVideo(videoURL: URL, atPhotoIndex index: Int) {
        ...
        if let player = currentVideoPlayerViewController.player {
            ...
            // Show
            present(currentVideoPlayerViewController, animated: false, completion: {
                player.play()
            })
        }
    }
    ...
    func clearCurrentVideo() {
        if let player = currentVideoPlayerViewController.player {
            ...
            currentVideoPlayerViewController.dismiss(animated: true)
